...
class GroupModel extends BaseModel{
    ...
    public function setValueGroup(string $name, string $leader, string $member)
    {
        try {
            ...
            if($this->checkDupNameGroup($name,$leader,$member)){
    ...
    private function checkDupNameGroup(string $name, string $leader, string $member)
    {
        $name = trim($name);
        $table = $this->tableGroup;
        $sql = "SELECT * FROM $table WHERE group_name='$name' AND group_leader='$leader' AND group_member='$member'";
        try {
